...
1. 咲夜優先の最小実装単位の確定
`madness-design-note.md` で整理した前提を受けて、次に切る最小単位を `狂気` の対象変更を `DokaanCard` と `RandomReflectionCard` の実挙動に限定して整理する小修正とする。
検証は `狂気なし` / `狂気あり` の対象数差を 2 枚で見る形に絞る。
...
- 次は「1. 咲夜優先の最小実装単位の確定」を優先し、`DokaanCard` と `RandomReflectionCard` に限定した小修正から着手する。
