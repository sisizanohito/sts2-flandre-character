...
- 次は「6. Spine 移行の入口整備」を優先し、`Enable-FlandreSpineVisuals.ps1` の前提になる素材・配置先・差し替え対象を明文化する。
- ただし automation では visual / animation 系を対象外にするため、非アセット系の次手は [card-pool-implementation-status.md](./card-pool-implementation-status.md) を起点に、既存 12 枚の整理か追加 1 枚の最小実装のどちらか 1 件に絞る。
