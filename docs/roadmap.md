...
1. パチュリー優先の設計整理
`狂気` と対象変更処理について、共通層依存をどこまで許容するかを先に固める。
`card-pool-draft.md` と既存コードを照らし合わせ、`破壊の目`、連鎖処理、対象変更まわりも含めて仕様を曖昧なまま進めない。
...
- 次は「3. 表示確認ポイントの定型化」を優先し、hover tip / keyword / pool 名 / 戦闘中表示の確認項目を install gate の後段として固定する。
