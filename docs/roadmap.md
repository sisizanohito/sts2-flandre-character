...
完了: [card-pool-implementation-status.md](./card-pool-implementation-status.md) を追加し、現在コード上に存在する starter / common / uncommon / rare のカードを class 単位で棚卸しした。
...
完了: [card-pool-package-map.md](./card-pool-package-map.md) に `CruelBlinkCard` を含む現在の common package 分類を readable な形で反映した。
...
- 非アセット系の次手は [card-pool-package-map.md](./card-pool-package-map.md) を起点に、eye lane か madness lane のどちらか片側だけを伸ばす追加 1 枚へ絞る。
- `Bloodshed` の消費カードとして `BloodPactCard` を追加済み。次の `Bloodshed` 追加は [bloodshed-design-note.md](./bloodshed-design-note.md) を起点に、「Nごとのスケーリング」だけへ絞る。
