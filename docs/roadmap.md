...
`狂気なし` / `狂気あり` の対象数差を `DokaanCard` と `RandomReflectionCard` の 2 枚で確認する小修正を次の直近実装とする。
共通層を広げず、2 枚の実挙動と最終ターゲティング差分だけを見る。
...
- 次は「1. DokaanCard / RandomReflectionCard に限定した最小実装単位」を優先し、`狂気なし` / `狂気あり` の対象数差を 2 枚で確認する。
